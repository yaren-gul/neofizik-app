...
function PreTest() {
  ...
  const [answers, setAnswers] = useState({}); // { [index]: selectedIdx }
    ...
    try {
      if (auth.currentUser) {
        ...
        await updateDoc(userRef, {
          ...
          preTestScore: calculatedScore,
          preTestCorrectCount: finalScoreCount,
          preTestTotalQuestions: total,
          preTestAnswerLog: finalLog,
        });
      }
      navigate('/results');
    } catch (error) {
  ...
  if (phase === 'intro') {
    return (
      <PhoneShell>
        <Screen align="center">
          <TopBar back={() => navigate('/info-form')} />
          ...
          <Card>
            <p style={{ fontFamily: font.body, fontSize: '13px', color: colors.text, lineHeight: 1.65, margin: 0 }}>
              Bu bölümde yenidoğanın fizik muayenesine ilişkin bilgi soruları bulunmaktadır.<br /><br />
          ...
          <div style={{ display: 'flex', gap: '10px', width: '100%', marginBottom: '14px' }}>
            ...
            <div style={{ flex: 1, backgroundColor: colors.card, border: `1px solid ${colors.tealBorder}`, borderRadius: '14px', padding: '14px', textAlign: 'center' }}>
              <div style={{ fontFamily: font.heading, fontSize: '13px', fontWeight: 700, color: colors.tealDark, marginTop: '3px' }}>Doğru–Yanlış</div>
              <div style={{ fontFamily: font.body, fontSize: '11.5px', color: colors.textMuted }}>Gösterilmez</div>
            </div>
          </div>

          <InlineNote tone="error">
            Ön test {questions.length} çoktan seçmeli sorudan oluşmaktadır. Yanıtlarınız eğitim tamamlanıncaya kadar doğru veya yanlış olarak gösterilmeyecektir.
          </InlineNote>

          <div style={{ flex: 1 }} />

          <PrimaryButton onClick={() => setPhase('test')}>Ön Teste Başla</PrimaryButton>
        </Screen>
      </PhoneShell>
    );
  }
  ...
  return (
    <PhoneShell>
      <Screen align="center">
        <TopBar badge="Ön Test" back={false} />
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: '12px', width: '100%', marginTop: '18px' }}>
              ...
              <PrimaryButton onClick={handleNext} icon={false}>
                {currentQuestionIndex === questions.length - 1 ? 'Ön Testi Tamamla' : 'Sonraki Soru'}
              </PrimaryButton>
            </div>
          </>
        ) : (
